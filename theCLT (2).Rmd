...
Describe the resulting plot -  is it unimodal?  How many peaks does it have?  Are all the peaks the same size? 
 ...
 Here we have a trimodel plot because it has three peaks. All of the peaks are not the same size, with the one in the middle being the highest. For this plot the mean is 49.8995 and the standard deviation is 29.29363.
...
print(sd(x.avg))
```
Here we have a trimodal plot. The peaks are not all the same size. The mean is 50.5675 and the standard deviation is 20.38927.
...
Here we see the plot moving to being more unimodal. The mean is 50.68622 and the standard deviation is 13.91124.
...
The plot is unimodal, meaning it only has one peak. The mean is 50.18228 and the standard deviation is 6.828936.
